...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.fine_tune:
            depthEstModel = import_depth_model((1024, 320), model_type='manydepth')
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            bs = all_color_aug.shape[0]
            Ks = torch.cat(bs * [self.models["DepthModelWrapper"].K])
            invKs = torch.cat(bs * [self.models["DepthModelWrapper"].invK])

            features, lowest_cost, _ = self.models["encoder"](current_image=all_color_aug,
                                         lookup_images=all_color_aug.unsqueeze(1)*0,
                                         poses=self.models["DepthModelWrapper"].zero_pose,
                                         K=Ks,
                                         invK=invKs,
                                         min_depth_bin=self.models["DepthModelWrapper"].min_depth_bin,
                                         max_depth_bin=self.models["DepthModelWrapper"].max_depth_bin)
            ...
            Ks = torch.cat(self.opt.batch_size*[self.models["DepthModelWrapper"].K])
            invKs = torch.cat(self.opt.batch_size*[self.models["DepthModelWrapper"].invK])
            features, lowest_cost, _ = self.models["encoder"](current_image=input_img,
                                         lookup_images=input_img.unsqueeze(1)*0,
                                         poses=self.models["DepthModelWrapper"].zero_pose,
                                         K=Ks,
                                         invK=invKs,
                                         min_depth_bin=self.models["DepthModelWrapper"].min_depth_bin,
    ...
    def save_model(self):
        ...
        for model_name, model in self.models.items():
            ...
            if model_name == 'encoder':
                ...
                to_save['min_depth_bin'] = self.models["DepthModelWrapper"].min_depth_bin
                to_save['max_depth_bin'] = self.models["DepthModelWrapper"].max_depth_bin
