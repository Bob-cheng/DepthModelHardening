...
from image_preprocess import process_car_img
from dataLoader import KittiLoader
from my_utils import ori_W, ori_H, object_dataset_root
...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.fine_tune:
            depthEstModel = import_depth_model((1024, 320), 'depthhints')
        ...
        if self.opt.adv_train and self.opt.supervised_adv:
            self.gt_model = import_depth_model((1024, 320), "depthhints").to(self.device)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        if self.opt.adv_train and self.opt.supervised_adv:
            ...
            # losses["loss"] = loss_sup ## for test purpose 
            # return losses
