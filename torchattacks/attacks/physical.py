...
class Physical(Attack):
    ...
    """
    def __init__(self, model, obj_img, obj_mask, adv_obj_img, adv_obj_mask,adam_lr=0.5, steps=10, mask_wt=0.1, l0_thresh=1/10, dist_range=list(range(5, 31, 2))):
        super().__init__("PGD", model)
        self.obj_img = obj_img.clone().detach()
        self.obj_mask = obj_mask.clone().detach()
        self.adv_obj_img = adv_obj_img.clone().detach()
        self.adv_obj_mask = adv_obj_mask.clone().detach()
    ...
    def forward(self, images, batch_size, cfg_path=f'{object_dataset_root}/training/calib/003086.txt', eval=False, color_jit=False):
        ...
        """
        phy_trans_adv = PhysicalTrans(self.adv_obj_img, self.obj_mask, self.conf, images.size())
        phy_trans_ben = PhysicalTrans(self.obj_img, self.obj_mask, self.conf, images.size())

        img_B,img_C,img_H,img_W = images.size()
        if img_H != ori_H or img_W != ori_W:
            images = self.resize2Ori(images)
            print("image size inconsistent in attack")
        ...
        obj_imgs_out_adv, obj_masks_out, _, _ = phy_trans_adv.project(batch_size=batch_size, z0_sample=z0_sample, alpha_sample=alpha_sample)
        adv_scenes = scene_imgs * (1 - obj_masks_out) + obj_imgs_out_adv * obj_masks_out
        adv_scenes = self.resize_trans(adv_scenes)

        obj_imgs_out_ben, _, _, _ = phy_trans_ben.project(batch_size=batch_size, z0_sample=z0_sample, alpha_sample=alpha_sample)
        ...
        return adv_scenes, ben_scenes, obj_masks_out, self.obj_img
